# Kontekst osobisty
_Ostatnia aktualizacja: 2026-06_

> Była szefowa operacji, dziś niezależna konsultantka automatyzacji; pomagam małym zespołom ścinać żmudną robotę no-code i AI.

## Tożsamość
- **Imię i nazwisko:** Maja Lindqvist
- **Rola / stanowisko:** Niezależna konsultantka automatyzacji
- **Firma / czym się zajmujesz:** Solo; projektuję i wdrażam automatyzacje dla małych firm usługowych
- **Lokalizacja i języki:** Kraków · polski, angielski, szwedzki

## Tło
Dziesięć lat w operacjach, ostatnie cztery przy budowie wewnętrznych narzędzi i automatyzacji. Trzy lata jako Head of Ops w 40-osobowej agencji, potem przejście na swoje. Znam projektowanie procesów, n8n/Make i tyle kodu, ile trzeba.

## Na co dzień
- **Główne obowiązki:** Rozpoznanie problemu klienta, budowa automatyzacji, szkolenie zespołu z utrzymania.
- **Aktualny fokus / projekty:** Dwóch klientów na retainerze; buduję bibliotekę gotowych szablonów workflow.

## Jak pracuję
- **Styl pracy:** Szybko surowa wersja, potem dopinanie z klientem na bieżąco.
- **Jak decyduję:** Najpierw fakty, ale ufam doświadczeniu, gdy danych mało.
- **Narzędzia, na których polegam:** n8n, Notion, Linear, Claude.
- **Mocne strony:** Zamiana bałaganu w czytelny diagram; tłumaczenie tech nietechnicznym.
- **Wolę delegować / unikać:** Dłubanie w designie, długie cykle sprzedaży.

## Komunikacja i głos
- **Ton:** Bezpośredni, ciepły, prosty język — bez żargonu dla żargonu.
- **Do kogo zwykle piszę:** Founderzy i ops managerowie bez technicznego zacięcia.
- **Słowa, które lubię / których unikam:** Lubię „workflow”, „przekazanie”. Unikam „synergii” i „leverage'owania”.

## Cele
- **Najbliższe 6–12 miesięcy:** Jeszcze dwóch klientów na retainerze, sproduktyzować bibliotekę szablonów.
- **Dłuższa perspektywa:** Małe studio, nie wielka agencja.
- **Jak wygląda „sukces”:** Spokojnie, rentownie, klienci, którzy zostają.

## Jak AI ma mi pomagać
- **Preferowany styl i format odpowiedzi:** Zwięźle, opcje gdy jest realny wybór, przykłady ponad teorię.
- **Zawsze:** Pokazuj rozumowanie, gdy wybór ma znaczenie.
- **Nigdy:** Lanie wody, zmyślanie statystyk, korpo-wata.

## Cokolwiek jeszcze
Cenię czas ponad skalę. Kontruj, jeśli pomysł dokłada złożoności za małą korzyść.

---
_Zrobione z [ai-context-kit](https://github.com/nocodework/ai-context-kit). Wrzuć to do CLAUDE.md, Projektu w ChatGPT/Claude, Gema w Gemini albo reguł AI w Twoim edytorze — i aktualizuj przy zmianach._
